"""Run Chance against a built-in Blizzard AI inside Docker.

Designed to be exec'd from run_docker.sh when the bot ships its own
run_vs_blizzard.py.  Imports the bot class directly, avoiding
module-name collisions between the runner's config.py and Chance's
own config module.
...
Reads the same environment variables as run_vs_computer.py:
  ...
  BOT_RACE                  - bot race (default: Random)
"""
...
def main() -> str:
    ...
    try:
        result: Result | list[Result | None] = run_game(
            map_data,
            [Bot(bot_race, bot_instance, "Chance"), opponent],
            realtime=False,
