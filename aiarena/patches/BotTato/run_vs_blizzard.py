"""Run BotTato against a built-in Blizzard AI inside Docker.

Designed to be exec'd from run_docker.sh when a bot ships its own
run_vs_blizzard.py.  Unlike run_vs_computer.py (dynamic import), this
script imports the bot class directly, avoiding sys.path / module-name
collisions (e.g. a runner ``config.py`` shadowing a bot's ``config`` module).
...
def main() -> str:
    ...
    try:
        result: Result | list[Result | None] = run_game(
            map_data,
            [Bot(bot_race, bot_instance, "BotTato"), opponent],
            realtime=False,
